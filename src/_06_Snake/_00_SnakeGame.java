...
public class _00_SnakeGame implements ActionListener, KeyListener {
    ...
    public void startGame() {
        ...
        String instructions = "Control the snake using the arrow keys. Each time the snake eats food, it will grow longer. Don't run into the walls, or your own tail!";
    ...
    private void gameOver() {
        Location snakeLoc = new Location(WIDTH / 2, HEIGHT / 2);
        //1. stop the timer
        timer.stop();
        //2. tell the user their snake is dead
        JOptionPane.showMessageDialog(null, "You killed the snake! Game over.");
        //3. ask them if they want to play again.
        String[] options = {"Yes", "No"};
        int input = JOptionPane.showOptionDialog(null, "Would you like to play again?", "Game over", 0, -1, null, options, 0);
        //4. if they want to play again
        //   reset the snake and the food and start the timer
        String choice = options[input];
        switch (choice) {
            case "Yes":
                snake.reset(snakeLoc);
                timer.start();
                break;
            case "No":
                window.dispose();
